...
```{r packages,echo=TRUE,comment=">",message=FALSE,warning=FALSE}
Mes_packages = c('rio',
                 'openxlsx','readxl',
                 'feather','arrow',
                 'data.table',
...
```{r plm_install,exercise=TRUE,comment=">",echo=FALSE}
...
```{r plm_data,exercise=TRUE,comment=">",echo=TRUE}
...
```{r wages_install,exercise=TRUE,comment=">",echo=TRUE}
...
```{r wages1,exercise=TRUE,comment=">",echo=TRUE}
...
Nous allons sélectionner un sous-ensemble des 1000 ouvriers avec plus de
10 ans d'expérience et moins de 10 ans d'études en 1980.

```{r wages_s,exercise=TRUE,comment=">",echo=TRUE}
Wages_p <- plm::pdata.frame(Wages,index=595)
head(Wages_p,7)
Wages_s <- subset(Wages_p, 
                  bluecol == "yes" & exp >=10 & ed < 10 & time==5 , 
                  select = c(exp,bluecol,sex,ed,lwage))
Wages_s <- Wages_s[order(Wages_s$lwage),]
str(Wages_s)
summary(Wages_s)
```
...
save(bf,file=here("data/bigfile.rds"))
file.info(here("data/bigfile.rds"))$size      
```
...
bench_size <- data.frame(Fichiers=c("data/bigfile.rds",
                 "data/bf1.csv",
                 "data/bf2.csv",
                 "data/bf3.csv",
                 "data/bf4.csv",
                 "data/bf5.csv"))
bench_size$Fonction <- c("base rds","base csv","readr","data.table","rio","vroom")
...
On constate en écriture que le fichier rds est le plu lent suivi du csv
, les autres format sont proches, le plus rapide est feather
...
suivi du format feather, les autres format sont proches, le plus léger
est parquet
...
On constate en lecture que le fichier rds est le plus lent suivi deu csv
